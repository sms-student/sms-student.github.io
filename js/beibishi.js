...
var beibishi = function() {
    ...
    var init = function() {
        ...
        option_div_1 = $("#option_div_1");
        option_div_2 = $("#option_div_2");
        option_div_3 = $("#option_div_3");
        option_div_4 = $("#option_div_4");
    ...
    var check_answer = function() {
        var WA = false;
        var ans_s = "";
        if (cur_prob.is_write) {
            var val = document.getElementById("option_write").value;
            if (val != cur_prob.write_answer) {
                WA = true;
                ans_s = cur_prob.write_answer;
            }
        } else {
            for (var i = 0; i < 4; i++) {
    ...
    var show_problem = function(p) {
        ...
        if (p.is_write) {
            document.getElementById("option_write").value = "";
            option_write.css("display", "")
            option_div_1.css("display", "none");
            option_div_2.css("display", "none");
            option_div_3.css("display", "none");
            option_div_4.css("display", "none");
        } else {
            option_write.css("display", "none")
            option_div_1.css("display", "");
            option_div_2.css("display", "");
            option_div_3.css("display", "");
            option_div_4.css("display", "");
        }
    };
    ...
    var load_problems = function() {
        var convert_problem = function(p) {
            ...
            if (ret.is_write) {
                ret.description = "（填空题）" + ret.description;
            }
            for (var i = 0; i < 4; i++) {
                ret.answer[i] = false;
            }
        ...
        var probs = [
            ...
            ["选手提交的源代码文件名是否有特殊要求", "源程序文件名由试题名称缩写加后缀构成", "试题名称缩写及后缀一律使用小写", "试题名称缩写及后缀一律使用大写", "试题名称首字母大写，后缀小写", true, "12", true],
            ["在NOI考试中，Pascal 源文件的扩展名规定为", "pas", "cpp", "c", "txt", false, "1", true],
            ["在NOI考试中，C 源文件的扩展名规定为", "c", "cpp", "pas", "txt", false, "1", true],
            ...
            ["在NOI上机考试中，是否允许选手私自重新启动计算机", "是", "否", "可能允许", "可能不允许", false, "2", true],
            ...
            ["在NOI上机考试中，允许选手使用的编程语言包括", "C", "C++", "Pascal", "C++11", true, "2", true],
            ...
            ["对评测结果中的超时有异议，且复测结果的运行时间与题目时间限制之差小于题目时间限制5％的申诉是否会被受理", "否", "是", "可能是", "可能否", false, "1", true],
            ...
            ["NOI 的中文意思是", "全国信息学奥林匹克竞赛", "全国信息学奥林匹克联赛", "国际信息学奥林匹克竞赛", "国际信息学奥林匹克联赛", false, "1", true],
            ...
            ["今年是第几届NOI？", "38", "37", "39", "36", false, "1", true],
            ["今年是第几届IOI？", "33", "32", "31", "30", false, "1", true],
            ["第 12 届IOI是哪一年在北京举办的", "2000", "1984", "1985", "2017", false, "1", true]
        ];
        all_probs = [];
